...
class Player(Ship):
    ...
    def draw(self, window):
        ...
        high_font = pygame.font.SysFont("comicsans", 30)
        super().draw(window)
        self.healthbar(window)
        score_label = main_font.render(f"Score: {self.score}", 1, (255,255,255))
        high_label = high_font.render(f"High Score: {high_score[0]}", 1, (255,255,255))
        WIN.blit(score_label, (WIDTH/2 - score_label.get_width()/2, 10))
        WIN.blit(high_label, (WIDTH/2 - high_label.get_width()/2, 50))
...
def main():
    ...
    def write_score():
        ...
        high_score_file.write(str(high_score[0]))
        high_score_file.close()
